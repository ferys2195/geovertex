...
export function ProjectCard({ project, onOpen, onDelete }: ProjectCardProps) {
  const role = project.my_role || "owner";
  const roleBadgeConfig = {
    owner: { label: "Owner", style: "bg-blue-500/15 text-blue-400 border-blue-500/30" },
    editor: { label: "Editor", style: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30" },
    viewer: { label: "Viewer", style: "bg-amber-500/15 text-amber-400 border-amber-500/30" },
  }[role] || { label: "Owner", style: "bg-blue-500/15 text-blue-400 border-blue-500/30" };
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold border ${roleBadgeConfig.style}`}>
